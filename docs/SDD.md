...
**Where this lives: `src/storage/s3.ts`.** It is the only module in the codebase that moves bytes, and the only one that reads the `S3_*` variables — from a single function, `s3ConfigFromEnv()`, so that "reads seven variables and nothing else" is a fact a test can check rather than a claim a reviewer has to re-derive. `tests/architecture/storage-speaks-s3.test.ts` is what turns the paragraphs above from documentation into a constraint. The two halves have deliberately different reach, and the difference matters when reading a failure: the MinIO check scans **all of `src/`**, because the interesting version of that mistake is a route handler importing `minio` directly rather than the store doing it, while the seven-variable check scans **`src/storage/`** — every file in it, not just `s3.ts`, since an eighth variable arrives in a new `config.ts` at least as easily as in the existing file. Both read source as text, so a specifier or a lookup assembled at runtime slips past; the tests say so in their own comments rather than implying otherwise. `tests/storage/s3.test.ts` proves the rest against a live MinIO — it `put`s an object and then `fetch`es the URL `url()` hands out, which is the only way to check that a browser can retrieve what the server wrote instead of restating that it can.

Two consequences of that portability are easy to miss. Object keys are opaque — `avatars/<uuid>.<ext>`, never derived from the uploaded filename — because a key of `Nguyễn Văn A.jpg` is both the percent-encoding case SigV4 implementations disagree about and a child's name written into server logs, proxy logs and browser history. And the bucket has to be *made* readable: it is private by default, so `compose.yaml`'s `storage-init` writes out a policy granting anonymous `s3:GetObject` on `arn:aws:s3:::<bucket>/*`. Skipping that gives correct URLs that a browser cannot fetch. It is written out in full rather than with `mc anonymous set download`, because that shorthand also grants `s3:ListBucket` — which turns an unauthenticated `?list-type=2` into a paginated index of every avatar, cover and condition photograph, and makes the opaque keys above worth nothing. The policy has exactly one definition, `tests/support/bucket-policy.ts`; the storage suite applies it and `tests/architecture/compose-grants-only-get-object.test.ts` asserts `compose.yaml` ships the same document, because it was two hand-maintained copies that let the two drift while three comments claimed they had not.
