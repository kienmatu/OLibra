...
| 3 | `gd` / `imagick`? | **ANSWERED 2026-09-01.** `php -m` returns `exif`, `fileinfo`, `gd`, `zip` — **`imagick` is ABSENT.** | Intervention Image runs on the **`gd` driver**; the imagick-preferred half of this row is moot. `exif` being present is what makes Phase 3c-i's EXIF-rotate and its GPS-strip test possible — but the dev container and both CI workflows installed `gd` and **not** `exif`. **DONE 2026-09-01 (Phase 3c-i Task 8):** `exif` added to `docker/php/Dockerfile:3`, `.github/workflows/laravel.yml:34` and `.github/workflows/deploy-laravel.yml:30`. One capability of this row stays UNCONFIRMED — whether the host's `gd` was compiled with WebP *encode* support — so `App\Support\Members\AvatarImage` asks `gd_info()['WebP Support']` at runtime and falls back to JPEG rather than assuming. |
...
| 6 | `symlink()` allowed? | **ANSWERED 2026-09-01.** `function_exists("symlink")` is `true`, and `disable_functions` is `system,passthru,shell_exec,escapeshellcmd,dl,show_source,posix_*` — **`symlink` is not on it, and neither is `exec`.** | `artisan storage:link` works, so `post-deploy.sh`'s `|| true` is belt-and-braces rather than the load-bearing fallback it was written as. **Note the shim docroot is a separate question** (see the option-3 note below): a symlink under `public/` is only served if the shim reaches it. **CONFIRMED 2026-09-01 (Phase 3c-i Task 8): under the shim it is NOT.** `deploy/post-deploy.sh:128-134` copies `public/build` and `public/.htaccess` into `$PUBLIC_HTML_PATH` and nothing else, so a `public/storage` symlink sits in a directory the web server never reaches. The avatar disk therefore takes this row's own fallback — `config/filesystems.php`'s `avatars` disk reads `AVATAR_DISK_ROOT`/`AVATAR_DISK_URL`, which under the shim are set to a path inside the served docroot (see `.env.example`). Under docroot options 1 and 2 the defaults are correct and `storage:link` is all that is needed. |
