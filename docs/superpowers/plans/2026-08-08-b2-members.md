...
**Rule 4 — a matched identity's personal fields are never overwritten.** This is BR §5.3's "only the parish details are re-entered" read strictly, and it is also INV-13 — which read, when this plan was written, "A person's verified details change only through an approved ProfileChangeRequest." *(BR §6 restated INV-13 on 2026-08-09: a manager's audited direct correction is a second sanctioned path. The rule below is unchanged — a registration form at a second parish is neither of the two paths — and this quotation is left as the wording this plan was written against.)* A family that moves and gives a new phone number at the new parish gets a new `memberships` row with its parish units and **the old phone number still in force**. Correcting it is `ProposeProfileChange` (B2b). Task 3's test asserts the old value survives, because writing the new one is the obvious wrong implementation.
