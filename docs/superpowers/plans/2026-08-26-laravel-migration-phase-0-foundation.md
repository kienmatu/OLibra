...
**Revision note (2026-08-26):** every piece of schema DDL in Tasks 6–12 was executed against a live MariaDB **10.11.19** server during review. That run changed the plan: uuid columns became `VARCHAR(36)` (a `CHAR(36)` operand inside a generated-column expression is refused with errno 1901), the two folded columns lost an illegal `STORED NOT NULL`, enum-backed columns gained `ascii_bin` so their CHECKs are case-exact, the fold map was extended beyond Vietnamese with a property-based parity test, hash keys gained a `CHAR_LENGTH` prefix, the tenant scope now fails closed, route-model binding is shelf-scoped, and sessions are stored under `sha256(id)`. The SQL in this plan is tested, not reasoned about.
...
- **Enums:** `VARCHAR(20) CHARACTER SET ascii COLLATE ascii_bin` + `CHECK (col IN (…))` + PHP backed enums in `app/Enums`, cast on the model. Never MariaDB's `ENUM` type. The binary collation is part of the guarantee: under the table default `utf8mb4_unicode_ci`, `'ACTIVE'` satisfies `CHECK (status IN ('active', …))` (verified on 10.11.19) and then explodes as a `ValueError` in every enum cast — the Postgres enum refused it, so the CHECK must too.
...
# Can this user create triggers and stored generated columns — probed with the
# EXACT expression shapes the schema uses (a VARCHAR(36) ascii_bin operand in an
# IF, the UNHEX(SHA2(CONCAT_WS(...))) hash key, unique indexes on both), not a
# toy expression that answers "yes" on a host where the real ones fail.
mysql -e "create table _probe (
  id int primary key,
...
# A REAL trigger, not a grant check: with binary logging on and no SUPER, this
# is where ERROR 1419 appears — a grant listing TRIGGER would still say yes.
...
Then set `"php": "^8.4"` in `composer.json`. No alternative: if `docs/HOSTING.md` row 1 recorded no PHP 8.4, Task 1 was a blocking stop and this task should not have started.
...
# Interim: Task 16 replaces this with hashed-database, the custom driver that
# stores sha256(session id) instead of the raw id.
...
### Task 3: `App\Support\Fold` — the folding rule as pure PHP, one table for both halves
...
- Produces: `App\Support\Fold::fold(string $value): string` and `Fold::MAP` (`array<string, string>`, lowercase code point → ascii), the single normalisation BR §12 requires. Task 4's `FoldExpression::sql()` is generated **from `Fold::MAP`**, so the PHP fold and the SQL fold are twins by construction — one table, two renderings — rather than two implementations kept honest only by a test. Task 19's factories and every later phase's search/slug code call `fold()`.

The TypeScript original (still in the working tree, the reference until cutover) is NFD-decompose → strip combining marks → đ/Đ→d → lowercase → non-alnum→space → trim. **The PHP twin deliberately does not use `Normalizer`.** The database side cannot decompose — MariaDB has no NFD — so a `Normalizer`-based PHP fold and the generated-column SQL would be *different functions*: identical over Vietnamese, silently divergent for every other accented Latin letter (`Kästner` → `kastner` in PHP but `k stner` in the DB — a book a reader can never find again, with no error anywhere). BR §12's actual requirement is that the *stored* normalisation and the *search* normalisation be identical, and in the new system both are PHP-or-DB — so the two halves are built from one explicit table instead. What this concedes against the TS original: input arriving in decomposed Unicode form (rare — macOS filenames, some IMEs) folds its combining marks to spaces on **both** halves identically instead of stripping them; store==search still holds. The corpus test keeps agreement with the TS over real NFC Vietnamese.

Two properties remain load-bearing. **đ→d stays an explicit entry**: đ (U+0111) is a distinct Vietnamese letter, not a d with a mark, and dropping the entry means searching *dat rung* never finds *Đất Rừng Phương Nam*. **The table covers Latin beyond Vietnamese** — Latin-1 Supplement and Latin Extended-A (`ä`, `ñ`, `ç`, `š`, `ø`, `æ`, `œ`, `ß`, `ð`, …) — because authors on a Vietnamese parish shelf are routinely European (`Erich Kästner`, `Antoine de Saint-Exupéry`), and an unmapped letter degrades to a space.
...
git commit -m "feat: the folding rule as one explicit table — vietnamese plus latin, php half"
```

---

### Task 4: `FoldExpression` — the generated-column SQL rendered from `Fold::MAP`, and the property-based parity test
...
**Interfaces:**
- Consumes: `App\Support\Fold::MAP` and `Fold::fold()` (Task 3).
- Produces: `App\Support\FoldExpression::sql(string $expression): string` — given a SQL expression (a backtick-quoted column, a `COALESCE(...)`, or a bare `?` placeholder), returns the MariaDB expression that folds it identically to `Fold::fold()`. Task 7's books migration embeds this in the two `GENERATED ALWAYS AS (…) STORED` columns.

MariaDB has no `unaccent` and forbids stored functions in generated columns, so the Postgres `olibra_fold()` becomes a generated nested-`REPLACE()` chain — spec §4.2 option 1. The chain is rendered from `Fold::MAP`, never written by hand: with 144 entries the expression is roughly 4 KB (the spec's "roughly 2 KB" grew with the Latin extension), ugly but emitted rather than maintained. `LOWER()` runs first (utf8mb4's case mapping folds the uppercase precomposed letters), then one `REPLACE` per map entry, then `REGEXP_REPLACE` collapses everything non-alphanumeric — disposing of any code point outside the map (CJK, combining marks, symbols) exactly as the PHP regex does.

**The escape hatch, named in advance (spec §4.2 option 2):** if the real host's MariaDB refuses the rendered expression inside a generated column (errno 1901 or an expression-length limit — the known 10.11 cause, a `CHAR(n)` operand, is already designed out by the VARCHAR(36) rule in Global Constraints), or if `FoldParityTest` cannot be made to pass on the host's 10.11.13, the fallback is **app-maintained folded columns**: plain `TEXT COLLATE utf8mb4_bin` columns written by a `saving` model observer calling `Fold::fold()`, with the same parity corpus kept as the guard. That decision is taken by editing Task 7's Step 3 to plain columns plus an observer in `Book::booted()` — and recording the switch in `docs/HOSTING.md` row 10 — not by improvising mid-migration. The condition that triggers it is a failing probe or a failing `FoldParityTest` **on the real host**, nothing vaguer; Task 1's survey probe and this task's probe test both exercise the actual expressions, so a host that would refuse them is discovered before any schema work.
...
- [ ] **Step 5: Write the property-based DB-vs-PHP parity test**

Create `tests/Feature/FoldParityTest.php`. This is the port of `tests/db/folding.test.ts`, upgraded from an eight-title corpus to a property test: an assertion over every map entry and a code-point sweep, so it *proves* agreement instead of asserting it over inputs that happen to be Vietnamese:
...
Expected: PASS (about 700 single-row SELECTs — a few seconds). If the sweep reports a disagreement on some code point, the fix is a `Fold::MAP` entry giving both halves the same target — or, only where the two engines' case mappings genuinely differ, an entry in the test's exclusion list with the reason. If the generated-column probe fails on the **real host** (it passes on 10.11.19), STOP: that is the escape-hatch condition — record it in `docs/HOSTING.md` row 10, switch Task 7 to observer-maintained columns as described above, and only then continue.
...
git commit -m "feat: render the mariadb fold from Fold::MAP and prove parity property-wise"
```
...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('audit_log', function (Blueprint $table) {
            ...
            $table->string('bookshelf_id', 36)->charset('ascii')->collation('ascii_bin')->nullable();   // null: cross-shelf acts
            $table->string('actor_id', 36)->charset('ascii')->collation('ascii_bin')->nullable();       // null: system actions
            $table->string('action');
...
it('enum checks are case-exact — the columns are ascii_bin, so ACTIVE is not active', function () {
    ...
    // it too.
    dbgExpectViolation(fn () => dbgShelf(['status' => 'ACTIVE']), 4025);
    $shelf = dbgShelf();
    $user = dbgUser();
    dbgExpectViolation(fn () => dbgMembership($shelf, $user, ['role' => 'Reader']), 4025);
});
...
- Produces, for every later task **exactly under these names**:
  - `App\Support\TenantContext` — request-scoped singleton with **three states**: bound (`set(Bookshelf $bookshelf, ?Membership $membership): void`), explicitly system-wide (`actSystemWide(): void` — the Laravel form of `systemContext()` in `src/domain/kernel/tenant.ts`), and unset, which is an **error to query under**. Accessors: `bookshelf(): ?Bookshelf`, `bookshelfId(): ?string`, `membership(): ?Membership`, `isSystemWide(): bool`, `clear(): void`.
...
Wide mass-assignment (`$guarded` listing only the database-computed columns) on the non-User models is deliberate: writes go through Action classes that assign explicit fields (spec §1.3), and the models carry 20-plus columns each — a `$fillable` transcription of every column is noise that drifts. Every generated column IS guarded, because `Model::create($other->toArray())` would otherwise put it in the INSERT and MariaDB refuses a value for a generated column with errno 1906. `User` is the exception with a narrow `$fillable`, because it is the one model bound to request-validated data in this phase (and `username_active` is thereby excluded automatically).
...
{
    return [App\Models\Feedback::class, App\Models\AuditLog::class];
}
...
Why the hashing is not optional, stated honestly (an earlier draft argued "a leaked backup already yields everything", which is false in the dimension that matters): with a database dump alone, hashed rows leak **no usable session** while raw rows leak **every live session id in plaintext**; and with a dump *plus `.env`* — same home directory, same cPanel full backup — raw rows plus `APP_KEY` give instant, silent impersonation of every signed-in user, super admins included. That is an authentication bypass, not a confidentiality loss, and thirty lines close it. What genuinely does survive from the old design regardless: revocation — the `users` provider excludes soft-deleted rows, so a trashed user's session stops resolving on the very next request (asserted below), and Task 6's FK hard-deletes session rows with the user.

One testing trap, named now: `phpunit.xml` forces `SESSION_DRIVER=array`, so the handler ships untested unless a test opts in explicitly — the last test in this task's suite does exactly that, and must not be deleted as "redundant with the driver config".
...
class LoginRequest extends FormRequest
{
    ...
    public function authenticate(): void
    {
        ...
        $stored = $user?->password_hash ?? self::dummyHash();
...
Register the driver in `app/Providers/AppServiceProvider.php` `boot()` (alongside what Task 17 adds there):
...
Session::extend('hashed-database', function (Application $app) {
    ...
    return new HashedDatabaseSessionHandler(
        $app['db']->connection($config->get('session.connection')),
...
Set `SESSION_DRIVER=hashed-database` in `.env` and `.env.example` (replacing Task 2's interim `database`).
...
The `where('bookshelf_id', …)` here is legal because Task 15's architecture grep names exactly two files in its `$allowed` list — `BookshelfScope` and this middleware. The exemption being a named list is the point: a third file filtering by shelf fails the build until it argues its way onto the list.

In `bootstrap/app.php`, register the alias AND the priority that puts tenant resolution ahead of route-model binding (without the priority entry, `SubstituteBindings` — a `web`-group middleware — runs first, and any scoped child binding would query under an unbound, fail-closed context):
...
- Produces: every named route of spec §6, English URIs, in the order the spec requires, with **scoped route bindings** (`->scopeBindings()` — `{book}` resolves through `$shelf->books()`, so a foreign shelf's slug is 404); three persistent layouts; shared Inertia props `{ auth: { user }, shelf: { id, slug, name } | null }`; `resources/js/lib/copy.ts` as the single home of client copy. Phase 1 replaces `under-construction` renders screen by screen without touching a route name or the binding behaviour.
...
it('puts the tenant middleware on every route that names {shelf}', function () {
    ...
    foreach (Route::getRoutes()->getRoutes() as $route) {
        ...
        expect($route->gatherMiddleware())
            ->toContain('tenant', "route without tenant middleware: {$route->uri()}");
    }
});
...
it('never binds another shelf\'s book — scoped bindings, proven on a unique slug', function () {
    ...
    $only = App\Models\Book::query()->create([
