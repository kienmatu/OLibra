...
return [
    ...
    */
    ...
    |
    | This config file is published (rather than left to the package default)
    | for one reason: the package ships `enabled` as null, and a null value
    | makes Inertia\DevTools\DevTools::enabled() fall back to
    | app()->environment('local'). That is not a safe default here. When
    | devtools is on, Inertia\DevTools\Http\Authorize::allows() short-circuits
    | to true for the local environment -- no authentication, no gate -- and
    | GET /_inertia/devtools/entries then serves recorded `propValues` to
    | anybody who can reach the port. That payload is every prop the app has
    | rendered recently. .env and .env.example both ship APP_ENV=local, so
    | the unsafe branch is the one every developer and every naively-deployed
    | copy lands on, and it bypasses the pending/blocked gate entirely.
    |
    | So the default is a hard `false`, both .env and .env.example carry
    | INERTIA_DEVTOOLS_ENABLED=false so the knob is visible, and turning it on
    | is an explicit per-machine opt-in in a local .env that is never
    | committed. Never commit it as true.
    |
    */
