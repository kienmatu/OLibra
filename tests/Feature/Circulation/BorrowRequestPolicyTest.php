...
/*
 * The binding itself, over HTTP. The real borrow-request routes are
 * Task 11's and Task 12's; CirculationArchitectureTest currently pins
 * their ABSENCE, so this probe deliberately lives outside the
 * shelves/{shelf} URL space and spells no path fragment that pin
 * matches ('requests/{', 'borrow-requests/{', 'handover') — and outside
 * shelves/{shelf} it is also outside RouteOrderTest's "every reader-area
 * route carries a role: gate" sweep, which it would otherwise violate.
 * The ResolveTenantMiddlewareTest probe is the precedent, Route::has and
 * all: Pest keeps one router for the whole run, so this registers once.
 * That the route survives into later files is the pollution that precedent
 * warns about, so the exposure was measured rather than reasoned about:
 * `pest tests/Feature/Circulation/BorrowRequestPolicyTest.php
 * tests/Feature/Architecture/RouteOrderTest.php
 * tests/Feature/Architecture/CirculationArchitectureTest.php
 * tests/Feature/Tenancy/RouteIsolationTest.php` — this file FIRST, so the
 * probe is registered before all three route sweeps run — is 33 passed.
 * When Tasks 11 and 12 land the real routes and turn that absence pin into
 * a presence pin, this probe should give way to the real URLs.
 */
beforeEach(function () {
    if (! Route::has('brp-test.probe')) {
        Route::middleware(['web', 'tenant'])->scopeBindings()
            ->get('/brp-binding-probe/{shelf}/{borrowRequest}', fn (Bookshelf $shelf, BorrowRequest $borrowRequest) => response()->json(['id' => $borrowRequest->id]))
            ->name('brp-test.probe');
    }
});
...
it('a guest with the tenant context entirely unset holds none of the five', function () {
    [, , , $request] = brpFix('dong-thap-brp-guest');
    $guest = User::factory()->create(['full_name' => 'Anna Người Lạ Qua Đường']);
    app(TenantContext::class)->clear();

    foreach (['create', 'cancel', 'approve', 'reject', 'handover'] as $ability) {
        expect(Gate::forUser($guest)->allows($ability, $request))->toBeFalse($ability);
    }
    expect(Gate::forUser($guest)->allows('create', BorrowRequest::class))->toBeFalse();
});
...
it('binds this shelf\'s own request, and 404s — never 403 — every other shape', function () {
    ...
    // Load-bearing for the third assertion below: the row is STILL IN THE
    // TABLE, so its 404 is soft deletion doing the work rather than the
    // row having ceased to exist. Without this line, dropping SoftDeletes
    // from BorrowRequest hard-deletes the row and the 404 below stays
    // green for the wrong reason — measured, not assumed.
    ...
    $this->actingAs($reader)
        ->get("/brp-binding-probe/{$shelfA->slug}/{$requestA->id}")
        ->assertOk()->assertJson(['id' => $requestA->id]);

    // A real request id, a real shelf, and the two do not belong together:
    // absence, not refusal. assertNotFound is the load-bearing half —
    // assertStatus(403) is what a policy-level ownership check would give.
    $this->actingAs($reader)
        ->get("/brp-binding-probe/{$shelfA->slug}/{$requestB->id}")
        ->assertNotFound();

    // Soft deletion is undo (BR §11): an undone row is gone from the URL space.
    $this->actingAs($reader)
        ->get("/brp-binding-probe/{$shelfA->slug}/{$trashed->id}")
        ->assertNotFound();

    // A guessed id that never existed is indistinguishable from the two above.
    $this->actingAs($reader)
        ->get("/brp-binding-probe/{$shelfA->slug}/0199a1b2-c3d4-7000-8000-000000000000")
        ->assertNotFound();
});
