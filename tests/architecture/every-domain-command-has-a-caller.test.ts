...
const EXEMPT = new Set<string>([
  ...
  // Task 11 (QA remediation) wired `addCopies` too ("fix(b1): wire "Thêm bản"
  // too" — a review round found the form posting with no `action` at all,
  // which silently discards a manager's input via the browser's default GET
  // rather than merely doing nothing). `addCopiesAction` calls it from
  // `src/app/tu-sach/[shelf]/quan-ly/actions.ts`, and the "Thêm bản" form on
  // `.../quan-ly/sach/[id]/page.tsx` posts to that action — this exemption is
  // removed now that a real form calls it.
]);
